...
export class Updater {
  ...
  current() {
    ...
    try { version = JSON.parse(fs.readFileSync(path.join(this.root, 'package.json'), 'utf8')).version || version; } catch { /* on garde celle du démarrage */ }
    return { version, ...(rec || { sha: null, date: null, message: null, source: 'inconnue' }) };
  }

  status() {
    const cur = this.current();
    if (!this.running) this.running = { ...cur, startedAt: Date.now() };
    // Une mise à jour n'est proposée que si GitHub est réellement plus récent que les fichiers :
    // sur le poste de développement, les fichiers peuvent être en avance sur le dernier relevé GitHub.
    const newer = (a, b) => (a && b ? new Date(a).getTime() > new Date(b).getTime() : true);
    const available = Boolean(this.latest) && (!cur.sha || (this.latest.sha !== cur.sha && newer(this.latest.date, cur.date)));
    ...
    setTimeout(() => {
      if (process.platform === 'win32') {
        ...
        if (fs.existsSync(vbs)) {
          // `timeout` refuse de tourner hors console : on attend avec ping, et on lance le .vbs quoi qu'il arrive.
          spawn('cmd.exe', ['/c', `ping -n 4 127.0.0.1 >nul & wscript.exe "${vbs}"`], { detached: true, stdio: 'ignore', windowsHide: true }).unref();
        } else {
          spawn(process.execPath, [path.join(this.root, 'server.js')], { detached: true, stdio: 'ignore', cwd: this.root, env: process.env, windowsHide: true }).unref();
        }
